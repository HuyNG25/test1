...
class MuonTraTaiSan(models.Model):
    ...
    don_vi_cho_muon_id = fields.Many2one('don_vi', string='Đơn vị cho mượn', required=True, ondelete='restrict')
    ...
    def _onchange_ma_don_muon_id(self):
        if self.ma_don_muon_id:
            ...
            self.don_vi_cho_muon_id = don_muon.don_vi_cho_muon_id
    ...
    @api.depends('don_vi_cho_muon_id', 'muon_tra_line_ids')
    def _compute_ds_tai_san_chua_muon(self):
        for record in self:
            ...
            ds_tai_san = self.env['phan_bo_tai_san'].search([
                ('don_vi_id', '=', record.don_vi_cho_muon_id.id),
