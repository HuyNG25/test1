...
class LuanChuyenTaiSan(models.Model):
    ...
    bo_phan_nguon = fields.Many2one('don_vi', string='Bộ phận hiện tại', required=True, ondelete='restrict')
    bo_phan_dich = fields.Many2one('don_vi', string='Bộ phận chuyển tới', required=True, ondelete='restrict')
        ...
        for record in records:
            if record.luan_chuyen_line_ids:
                for line in record.luan_chuyen_line_ids:
                    ...
                    if phan_bo_tai_san:
                        # Cập nhật don_vi_id sang bộ phận đích
                        phan_bo_tai_san.write({
                            'don_vi_id': record.bo_phan_dich.id,
