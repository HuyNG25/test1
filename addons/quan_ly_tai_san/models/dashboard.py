...
class AssetDashboard(models.Model):
    ...
    def get_overview_data(self):
        ...
        in_use_assets = self.env['tai_san'].search_count([('don_vi_su_dung_ids.trang_thai', '=', 'in-use')])
        not_used_assets = self.env['tai_san'].search_count([('don_vi_su_dung_ids.trang_thai', '=', 'not-in-use')])
        ...
        # Tổng số tài sản theo Đơn vị
        departments = self.env['don_vi'].search([])
        departments_data = []
        for dept in departments:
            asset_count = self.env['phan_bo_tai_san'].search_count([('don_vi_id', '=', dept.id)])
            if asset_count > 0:
                departments_data.append({
                    'name': dept.ten_don_vi or dept.ma_don_vi,
                    'count': asset_count
                })
    ...
    def get_borrowing_data(self):
        ...
        if phan_bo_counts:
            ...
            for phan_bo_id, count in sorted_phan_bo:
                ...
                top_borrowed_assets.append({
                    ...
                    'department': phan_bo.don_vi_id.ten_don_vi or phan_bo.don_vi_id.ma_don_vi,
                    'count': count
                })
        ...
        for borrow in overdue_borrows:
            overdue_data.append({
                ...
                'department': borrow.don_vi_cho_muon_id.ten_don_vi or borrow.don_vi_cho_muon_id.ma_don_vi,
