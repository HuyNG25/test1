...
class PhongHopPermission(models.Model):
    ...
    @api.constrains('permission_type', 'nhan_vien_id', 'don_vi_id', 'chuc_vu_id')
    def _check_permission_target(self):
        ...
        for record in self:
            ...
            elif record.permission_type == 'don_vi' and not record.don_vi_id:
                raise ValidationError("Vui lòng chọn Đơn vị!")
    ...
    def _onchange_permission_type(self):
        ...
        if self.permission_type == 'user':
            self.don_vi_id = False
            self.chuc_vu_id = False
        elif self.permission_type == 'don_vi':
