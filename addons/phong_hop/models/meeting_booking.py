...
class MeetingBooking(models.Model):
    ...
    tai_san_muon_ids = fields.Many2many(
        ...
        help='Chọn tài sản cần mượn từ Đơn vị quản lý phòng họp'
    )
    ...
    def _onchange_meeting_room_id(self):
        """Filter tài sản theo Đơn vị quản lý của phòng họp"""
        if self.meeting_room_id and self.meeting_room_id.don_vi_quan_ly_id:
            return {
                'domain': {
                    'tai_san_muon_ids': [('don_vi_id', '=', self.meeting_room_id.don_vi_quan_ly_id.id)]
                }
            }
        else:
            return {
    ...
    def _create_asset_borrowing(self, initial_state='dang-cho'):
        ...
        # 2. Phòng có Đơn vị quản lý
        # 3. Chưa có đơn mượn (tránh duplicate)
        if not self.tai_san_muon_ids:
            return  # Không chọn tài sản → không cần tạo đơn mượn
        
        if not self.meeting_room_id.don_vi_quan_ly_id:
            raise ValidationError(
                f"Phòng họp '{self.meeting_room_id.name}' chưa có Đơn vị quản lý! "
                "Vui lòng cấu hình Đơn vị quản lý trước."
            )
        
        if self.don_muon_tai_san_id:
            return  # Đã có đơn mượn rồi
        
        don_vi_cho_muon = self.meeting_room_id.don_vi_quan_ly_id
        ...
        don_muon = self.env['don_muon_tai_san'].create({
            ...
            'don_vi_cho_muon_id': don_vi_cho_muon.id,
    ...
    def _user_can_approve(self):
        ...
        # Check theo Đơn vị
        if employee and hasattr(employee, 'don_vi_id') and employee.don_vi_id:
            if Permission.search([
                ('permission_type', '=', 'don_vi'),
                ('don_vi_id', '=', employee.don_vi_id.id),
                ('can_approve', '=', True)
            ], limit=1):
                return True
